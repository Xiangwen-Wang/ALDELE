...
class ConPrediction(nn.Module):
    def __init__(self, device, n_fingerprint, dim, layer_gnn, layer_cnn, window, layer_nn, layer_output):
    ...
    def attention_cnn(self, x, xs, layer):
        ...
        # print(max_attention)
        if max_attention == 0:
            attention_profiles = [0 for attention in attention_weights]
        else:
    ...
    def forward(self, inputs):
        ...
        fingerprints, adjacency, rdkitfeatures = inputs
        ...
        substrate_vector, attention_profiles = self.attention_cnn(rdkitfeature_vector, compound_vector, layer_cnn)

        compound_vector = torch.unsqueeze(torch.mean(compound_vector, 0), 0)
        cat_vector = torch.cat((substrate_vector, compound_vector), 1)
    ...
    def __call__(self, data, train=True):
        ...
        predicted_interaction, weighte = self.forward(inputs)
...
if __name__ == "__main__":
    ...
    dataset = list(zip(compounds, adjacencies, rdkitfeatures, interactions))
    ...
    model = ConPrediction(device, n_fingerprint, dim, layer_gnn, layer_cnn, window, layer_nn, layer_output).to(
        device)
    trainer = Trainer(model)
    tester = Tester(model)

    """Output files."""
    file_MAEs = DATASET+'/output/results.txt'
    file_model = DATASET+'/output/model'
