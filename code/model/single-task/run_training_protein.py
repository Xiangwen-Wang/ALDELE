...
class ConPrediction(nn.Module):
    def __init__(self, device, n_word, dim, layer_cnn, window, layer_nn, layer_output):
    ...
    def forward(self, inputs):
        ...
        protein_vector = self.attention_cnn(pssm_vector,
                                            word_vectors, layer_cnn)

        """Concatenate the above three vectors and output the interaction."""

        cat_vector = torch.cat((protein_vector, pssm_vector), 1)
...
if __name__ == "__main__":
    ...
    """Hyperparameters."""
    (DATASET, radius, ngram, dim, window, layer_cnn, layer_nn, layer_output,
    ...
    dataset = list(zip(proteins, pssms, interactions))
    ...
    model = ConPrediction(device, n_word, dim, layer_cnn, window, layer_nn, layer_output).to(device)
    trainer = Trainer(model)
    tester = Tester(model)

    """Output files."""
    file_MAEs = DATASET+'/output_weights/results.txt'
    file_model = DATASET+'/output_weights/model'
